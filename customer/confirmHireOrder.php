    ...
    if($chargeResponse['paid'] == 1 && $chargeResponse['captured'] == 1) {
        ...
        if (count($productID)>0) {
				...
				if ( $result->num_rows > 0 ) {
					...
					$orderSQL = "INSERT INTO `order`(`ID`,`order_type`, `userID`, `payment_status`, `orderStatus`) VALUES ($orderID, 2, $userID, 1, 0)";
                    echo $orderSQL."<br/>";
					$con->query( $orderSQL );
					...
					for($x=0;$x<count($productID);$x++){
						...
						$orderItemSQL = "INSERT INTO `hireproduct`(`OrderID`, `ProductID`, `qty`, `days`) VALUES ($orderID, $productID[$x], $qty[$x], $days[$x])";
                        echo $orderItemSQL."<br/>";
						$con->query( $orderItemSQL );

					}

					//now remove item form cart
					$delSQL="DELETE FROM `cart` WHERE `userID`=$userID AND hireId=1";
                    echo $delSQL."<br/>";
					$con->query( $delSQL );
